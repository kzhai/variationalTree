...
class Inferencer():
    ...
    def update_alpha(self, alpha_sufficient_statistics, number_of_documents):
        ...
        for alpha_iteration in xrange(self._alpha_maximum_iteration):
            ...
            alpha_gradient = number_of_documents * (scipy.special.psi(alpha_sum) - scipy.special.psi(self._alpha)) + alpha_sufficient_statistics;
            alpha_hessian = -number_of_documents * scipy.special.polygamma(1, self._alpha);
    ...
    def format_output(self, input_directory):
        ...
        if self._update_hyper_parameter:
            ...
            print "successfully updated hyperparameter alpha to", self._alpha
    ...
    def __call__(self, data):
        ...
        if self._hybrid_mode:
            ...
            for key, value in data:
                ...
                #gamma[doc_id] = self._alpha + topic_sum;
                #for topic_index in xrange(self._number_of_topics):
                    #yield (-doc_id-1, -topic_index-1), self._alpha[0, topic_index] + topic_sum[0, topic_index];
                #yield (-doc_id-1, -topic_index-1), " ".join(["%f" % item for item in (self._alpha + topic_sum)[0, :]]);
                
                '''
                document_level_log_likelihood += scipy.special.gammaln(numpy.sum(self._alpha)) - numpy.sum(scipy.special.gammaln(self._alpha));
                document_level_log_likelihood += numpy.sum((self._alpha - 1) * self.compute_dirichlet_expectation(gamma[[doc_id], :]));
            ...
            for key, value in data:
                ...
                for iteration_index in xrange(self._gamma_maximum_iteration):
                    E_log_gamma = self.compute_dirichlet_expectation(topic_sum + self._alpha);
                    topic_sum = numpy.zeros((1, self._number_of_topics));
                ...
                '''
                document_level_log_likelihood += scipy.special.gammaln(numpy.sum(self._alpha)) - numpy.sum(scipy.special.gammaln(self._alpha));
                document_level_log_likelihood += numpy.sum((self._alpha - 1) * self.compute_dirichlet_expectation(self._gamma[[doc_id], :]));
