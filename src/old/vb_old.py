...
class UncollapsedVariationalBayes(object):
    # 
    ...
    def _initialize(self, num_topics, data):
        ...
        self._number_of_documentsumber_of_documents = len(self._data)
        ...
        self._log_beta = defaultdict(dict)
        for v in self._vocab:
            temp = {}
            for k in range(self._K):
                temp[k] = log(1.0 / self._V + random())
            self._log_beta[v] = temp
    ...
    def inference(self):
        ...
        likelih_number_of_documentsd_alpha *= self._number_of_documents
        ...
        for doc in self._data.keys():
            ...
            for gamma_iteration in range(self._gamma_maximum_iteration):
                ...
                [gamma_update, phi_table, likelihood_phi_temp] = self.update_phi(self._data[doc], phi_table, self._log_beta, gamma, gamma_update)
        ...
        self._log_beta = self.normalize_beta(beta_normalize_factor, beta)
    ...
    def update_phi(self, doc, phi_table, beta, gamma, gamma_update):
        ...
        for term in doc.keys():
            ...
            for k in range(self._K):
                ...
                #phi[k] = self._log_beta[term][k] + sp.special.psi(self._gamma[doc][k])
    ...
    def update_alpha(self, alpha_vector, alpha_sufficient_statistics):
     ...
     _number_of_documents         alpha_gradient[k] = self._number_of_documents * (psi(alpha_sum) - psi(alpha_vector[k])) + alpha_sufficient_statistics[k]
                # compute the a_number_of_documentsha hessian
                alpha_hessian[k] = -self._number_of_documents * polygamma(1, alpha_vector[k])
...
_number_of_documents              sum_1_h += 1.0 / alpha_hessian[k]

            z = self._number_of_documents * polygamma(1, alpha_sum)
...
if __name__ == "__main__":
    ...
    d = import_monolingual_data("../../data/test/doc.dat");
